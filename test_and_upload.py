...
# Version: 2.9 (Fixed nested IndentationError in Base64 processing)
...
REQUEST_HEADERS = {
    'User-Agent': f'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36 KeyTester/2.9' # Version bump
}
...
def test_v2ray_key(key_url):
    ...
    try:
        ...
        if process.returncode == 0 and re.search(success_pattern, stdout_data, re.IGNORECASE):
            ...
            elif "timeout" in stdout_data.lower() or "timeout" in stderr_data.lower(): final_fail_reason = f"Check Timeout"
            elif stderr_data: final_fail_reason = f"Stderr (Exit 0): {stderr_data.splitlines()[0]}"[:150]
            elif stdout_data: final_fail_reason = f"No Success Pattern: {stdout_data.splitlines()[0]}"[:150]
            else: final_fail_reason = f"Exit {process.returncode}, No Output"
        return key_url, is_working
    except subprocess.TimeoutExpired: final_fail_reason = f"Subprocess Timeout"; print(f"ERROR: {final_fail_reason} Key: {key_url[:50]}...", file=sys.stderr); return key_url, False
    except FileNotFoundError: final_fail_reason = f"Not Found {abs_xray_knife_path}"; print(f"ERROR: {final_fail_reason}", file=sys.stderr); return key_url, False
    except Exception as e_test: final_fail_reason = f"Unexpected check error: {e_test}"; print(f"ERROR: {final_fail_reason} Key: {key_url[:50]}...", file=sys.stderr); traceback.print_exc(file=sys.stderr); return key_url, False
    finally:
        if not is_working:
             log_message = f"DEBUG: FAIL Key: {key_url[:60]}... Reason: {final_fail_reason}"
             if stderr_data and "Stderr" not in final_fail_reason and "Exit Code" not in final_fail_reason: log_message += f" | Stderr: {stderr_data.splitlines()[0]}"[:150]
...
def main():
    ...
    print(f"\n=== Starting Key Tester (v2.9 - fixed nested indent) at {time.strftime('%Y-%m-%d %H:%M:%S %Z')} ===") # Version bump

    # Step 1: Setup
    if not download_and_extract_xray_knife(): print("FATAL: Setup failed.", file=sys.stderr); sys.exit(1)
    abs_path = os.path.abspath(XRAY_KNIFE_PATH)
    if not os.path.exists(abs_path) or not os.access(abs_path, os.X_OK): print(f"FATAL: Not ready: {abs_path}.", file=sys.stderr); sys.exit(1)
    print(f"Confirmed executable: {abs_path}")

    # Step 2: Output Dir
    print(f"\n--- Step 2: Output Dir ({OUTPUT_DIR}) ---")
    try: os.makedirs(OUTPUT_DIR, exist_ok=True); print(f"Output dir ok.")
    except OSError as e: print(f"FATAL: Cannot create dir {OUTPUT_DIR}: {e}", file=sys.stderr); sys.exit(1)
    ...
    for index, url in enumerate(SOURCE_URLS_LIST):
        ...
        try:
            ...
            try: raw_data = response.content.decode('utf-8'); print(f"  Decoded UTF-8.")
            except UnicodeDecodeError:
                 try: encoding = response.encoding if response.encoding else response.apparent_encoding; encoding = encoding if encoding else 'iso-8859-1'; raw_data = response.content.decode(encoding, errors='replace'); print(f"  Decoded {encoding}.")
            ...
            count_for_source = 0
            for line in lines:
                line = line.strip() # Indent level 1 (inside 'for line')
                if line:             # Indent level 1 (inside 'for line')
                    # Indent level 2 (inside 'if line')
                    all_fetched_keys_raw.append(line)
                    count_for_source += 1
            # --- END CORRECTION ---
            total_lines_fetched += count_for_source
            print(f" -> Fetched {count_for_source} lines.")
        except requests.exceptions.Timeout: print(f"ERROR: Timeout {url[:100]}", file=sys.stderr); fetch_errors += 1
        except requests.exceptions.RequestException as e: print(f"ERROR: Fetching {url[:100]}: {e}", file=sys.stderr); fetch_errors += 1
    ...
    if not all_fetched_keys_raw:
        ...
        try: with open(OUTPUT_FILE_PATH, 'w') as f: pass; print(f"Created empty: {OUTPUT_FILE_PATH}")
        except IOError as e_f: print(f"Warning: Cannot create empty file: {e_f}", file=sys.stderr)
        print(f"Exiting. Errors: {fetch_errors}, Sources: {len(SOURCE_URLS_LIST)}"); sys.exit(0 if fetch_errors < len(SOURCE_URLS_LIST) else 1)
            ...
            try: # Start try block for Base64 decode
                ...
                if found_keys:
                    ...
                    for key in found_keys:
                        key = key.strip() # Indent level 1 (inside 'for key')
                        # Check protocol match
                        if any(key.startswith(proto) for proto in SUPPORTED_PROTOCOLS): # Indent level 1
                            # Indent level 2 (inside 'if any(...)')
                            unique_keys_to_test.add(key)
                            keys_found_in_base64 += 1
                    # *** END CORRECTION ***
            # Add except blocks back
            except (base64.binascii.Error, UnicodeDecodeError): skipped_invalid_lines += 1
            except Exception as e_dec: print(f"Warning: Base64 process error: {e_dec}", file=sys.stderr); skipped_invalid_lines += 1
    unique_keys_list = list(unique_keys_to_test)
    print(f"Processed: {processed_line_count}. Unique found: {len(unique_keys_list)}."); print(f"(Base64: {decode_attempts} attempts, {keys_found_in_base64} found. Skipped: {skipped_invalid_lines})")

    # Step 5: Test Keys
    print("\n--- Step 5: Testing Keys ---")
    if not unique_keys_list:
        print("No keys to test. Writing empty file."); try: with open(OUTPUT_FILE_PATH, 'w') as f: pass; print(f"Created empty: {OUTPUT_FILE_PATH}"); except IOError as e_f: print(f"Warn: Cannot create empty file: {e_f}", file=sys.stderr); sys.exit(0)
    print(f"Testing {len(unique_keys_list)} keys (Target: {TARGET_EARLY_EXIT_KEYS})..."); print(f"(Workers: {MAX_WORKERS}, Timeout: {TEST_TIMEOUT}s)")
            ...
            try:
                ...
                if is_working:
                    ...
                    if not stop_early and found_count >= TARGET_EARLY_EXIT_KEYS:
                        stop_early = True; print(f"\nTarget {TARGET_EARLY_EXIT_KEYS} reached! Stopping."); print("Cancelling...")
                        cancelled_now = 0
                        for f_obj in active_futures:
                            if not f_obj.done():
                                if f_obj.cancel(): cancelled_now += 1
                        futures_cancelled = cancelled_now; print(f"Cancelled {futures_cancelled}.")
    ...
    print(); print(f"Test finished. Processed: {tested_count}."); test_duration = time.time() - start_test_time; print(f"Duration: {test_duration:.2f}s.")
    ...
    random.shuffle(all_working_keys); print(f"Shuffled {num_working}.")
    keys_to_write = all_working_keys[:TARGET_EARLY_EXIT_KEYS] if num_working > TARGET_EARLY_EXIT_KEYS else all_working_keys
    num_to_write = len(keys_to_write); print(f"Writing: {num_to_write}.")
    try:
        with open(OUTPUT_FILE_PATH, 'w', encoding='utf-8', newline='\n') as f:
            for key in keys_to_write: f.write(key.replace('\r\n', '\n').replace('\r', '\n') + '\n')
        print(f"Wrote to: {OUTPUT_FILE_PATH}"); print(f"Abs path: {os.path.abspath(OUTPUT_FILE_PATH)}")
    ...
    print(f"Keys COLLECTED: {num_working}"); print(f"Keys WRITTEN: {num_to_write}"); print(f"Output: {os.path.abspath(OUTPUT_FILE_PATH)}")
    print(f"Finished in {total_time:.2f} seconds."); print("======================================================")
...
if __name__ == "__main__":
    ...
    except Exception as e_signal: print(f"Warning: Could not set signals ({e_signal}).")
    main()
