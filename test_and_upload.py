...
def download_and_extract_xray():
    ...
    try:
        ...
        if github_token:
            ...
            print("Warning: GH_TOKEN/GITHUB_TOKEN not found. Making unauthenticated API request.")
        ...
        if system != 'windows':
            try:
                ...
                except Exception as fallback_e: print(f"ERROR: Fallback chmod failed: {fallback_e}. Cannot guarantee Xray will run.", file=sys.stderr); return False
...
def test_v2ray_key(key_url):
    ...
    except Exception as e_pre: print(f"Warning: Pre-check error for {key_url[:30]}...: {e_pre}", file=sys.stderr); pass
    ...
    try: # Main test block
        ...
        time.sleep(2.5)
        if xray_proc.poll() is not None: return key_url, False
        ...
        for test_url in TEST_URLS:
            ...
            except requests.exceptions.ProxyError: is_working = False; break
