...
# Description: Fetches V2Ray/Xray keys, tests using xray-knife check,
#              handles early exit, saves working keys.
# Version: 3.0 (Major cleanup: Fixed all identified Syntax/Indentation errors, removed semicolons)
...
REQUEST_HEADERS = {
    'User-Agent': f'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Safari/537.36 KeyTester/3.0' # Version bump
}
...
def download_and_extract_xray_knife():
    ...
    try:
        ...
        if github_token:
            ...
            print("Using GH_TOKEN for API request.")
        else:
            print("Warning: GH_TOKEN not found.")
        ...
        for asset in release_info.get('assets', []):
            if asset.get('name') == asset_name:
                ...
                break
        if not asset_url:
            raise ValueError(f"Asset '{asset_name}' not found in release '{tag_name}'. Available: {available_assets}")
                    ...
                    if member_base_name == exe_name and not member.startswith('__MACOSX'):
                        ...
                        if os.path.abspath(extracted_path) != abs_xray_knife_path:
                            ...
                            if os.path.exists(abs_xray_knife_path) or os.path.islink(abs_xray_knife_path):
                                print(f"  Removing existing: {abs_xray_knife_path}")
                                os.remove(abs_xray_knife_path)
                            os.rename(extracted_path, abs_xray_knife_path)
                        else:
                            print(f"  Extracted to target path: {abs_xray_knife_path}")
                        ...
                        if member_dir and os.path.exists(os.path.join(".", member_dir)) and not os.listdir(os.path.join(".", member_dir)):
                            try:
                                ...
                                print(f"  Removed empty source dir: {os.path.join('.', member_dir)}")
                            except OSError as rmdir_e:
                                print(f"  Warning: Could not remove source dir {os.path.join('.', member_dir)}: {rmdir_e}")
                        print(f"  Executable placed at: '{abs_xray_knife_path}'")
                        extracted = True
                        break
                if not extracted:
                    raise FileNotFoundError(f"'{exe_name}' not found in {asset_name}. Contents: {zf.namelist()}")
        else:
             raise NotImplementedError(f"Extraction not implemented for: {asset_name}")
        if not os.path.exists(abs_xray_knife_path):
            raise FileNotFoundError(f"Executable not found at '{abs_xray_knife_path}' post-extraction.")
        if system != 'windows':
            try:
                print(f"Setting execute permissions...")
                current_mode = os.stat(abs_xray_knife_path).st_mode
                new_mode = current_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH
                if current_mode != new_mode:
                    os.chmod(abs_xray_knife_path, new_mode)
                    print(f"Permissions set: {oct(new_mode)}.")
                else:
                    print("Execute permissions ok.")
                if not os.access(abs_xray_knife_path, os.X_OK):
                    raise OSError("Execute check failed.")
        ...
        try:
            version_process = subprocess.run(
                [abs_xray_knife_path, "-v"], capture_output=True, text=True,
                timeout=15, check=False, encoding='utf-8', errors='replace'
            )
            print(f"--- VERSION ---")
            print(f"Cmd: {' '.join(version_process.args)}")
            print(f"Code: {version_process.returncode}")
            ...
            print(f"Out: {stdout_strip}")
            if stderr_strip:
                print(f"Err: {stderr_strip}")
            print(f"--- END ---")
            if version_process.returncode != 0 or "xray-knife" not in stdout_strip.lower():
                print("Warning: Version check failed.", file=sys.stderr)
            else:
                print("xray-knife version ok.")
        except subprocess.TimeoutExpired:
            print(f"ERROR: Timeout verifying.", file=sys.stderr)
            return False
        except FileNotFoundError:
            print(f"ERROR: Cannot execute.", file=sys.stderr)
            return False
        except Exception as verify_e:
            print(f"ERROR: Verification failed: {verify_e}", file=sys.stderr)
            traceback.print_exc(file=sys.stderr)
            return False
        print("xray-knife setup ok.")
        return True
    except requests.exceptions.RequestException as req_e:
        print(f"ERROR: Download failed: {req_e}", file=sys.stderr)
        return False
    except zipfile.BadZipFile as zip_e:
        print(f"ERROR: Bad ZIP: {zip_e}", file=sys.stderr)
        return False
    except (ValueError, NotImplementedError, FileNotFoundError, OSError) as setup_e:
        print(f"ERROR: Setup failed: {setup_e}", file=sys.stderr)
        return False
    except Exception as e:
        print(f"ERROR: Unexpected setup error: {e}", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)
        return False
...
def test_v2ray_key(key_url):
    ...
    try:
        ...
        if not host:
             final_fail_reason = "No host"
             return key_url, False
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(3.0)
            s.connect((host, port))
    except socket.gaierror:
        final_fail_reason = "DNS fail"
        return key_url, False
    except (socket.timeout, ConnectionRefusedError, OSError) as e_sock:
        final_fail_reason = f"Conn fail ({type(e_sock).__name__})"
        pass # Continue to main test
    except Exception as e_pre_other:
        print(f"Warn: Pre-check err {key_url[:30]}: {e_pre_other}", file=sys.stderr)
        pass
    ...
    try:
        ...
        if process.returncode == 0 and re.search(success_pattern, stdout_data, re.IGNORECASE):
            ...
            if process.returncode != 0:
                ...
                final_fail_reason = f"Exit {process.returncode}, No Output"
...
def main():
    ...
    print(f"\n=== Starting Key Tester (v3.0 - syntax/style cleanup) at {time.strftime('%Y-%m-%d %H:%M:%S %Z')} ===") # Version bump
    ...
    # Check if keys fetched - Corrected block structure
    ...
    if not all_fetched_keys_raw:
        ...
        try:
            ...
            print(f"Created empty: {OUTPUT_FILE_PATH}") # Separate line
        except IOError as e_f:
            print(f"Warning: Cannot create empty file: {e_f}", file=sys.stderr)
        print(f"Exiting. Errors: {fetch_errors}, Sources: {len(SOURCE_URLS_LIST)}") # Separate line
        sys.exit(0 if fetch_errors < len(SOURCE_URLS_LIST) else 1) # Separate line
    ...
    if not unique_keys_list:
        ...
        try:
            ...
            print(f"Warn: Cannot create empty file: {e_f}", file=sys.stderr)
        sys.exit(0) # Separate line
    ...
    if stop_early:
        print(f"({futures_cancelled} tests cancelled)")

    # Step 6: Write Results
    print("\n--- Step 6: Writing Results ---")
    num_working = len(all_working_keys)
    print(f"Collected: {num_working} (Target: {TARGET_EARLY_EXIT_KEYS})")
    random.shuffle(all_working_keys) # Separate statement
    print(f"Shuffled {num_working}.") # Separate statement
    ...
    print(f"Writing: {num_to_write}.") # Separate statement
    try:
        with open(OUTPUT_FILE_PATH, 'w', encoding='utf-8', newline='\n') as f:
            for key in keys_to_write:
                f.write(key.replace('\r\n', '\n').replace('\r', '\n') + '\n')
        print(f"Wrote to: {OUTPUT_FILE_PATH}") # Separate statement
        print(f"Abs path: {os.path.abspath(OUTPUT_FILE_PATH)}") # Separate statement
    ...
    print(f"Keys COLLECTED: {num_working}") # Separate statement
    print(f"Keys WRITTEN: {num_to_write}") # Separate statement
    print(f"Output: {os.path.abspath(OUTPUT_FILE_PATH)}") # Separate statement
...
if __name__ == "__main__":
    ...
    def handle_signal(sig, frame):
        ...
        print(f"\nSignal {sig} received. Exiting...") # Separate statement
        sys.exit(1) # Separate statement

    # Set up signal handlers - Corrected structure
    try:
        signal.signal(signal.SIGINT, handle_signal)
        signal.signal(signal.SIGTERM, handle_signal)
        print("Signal handlers set.") # Separate statement
    except Exception as e_signal:
        print(f"Warning: Could not set signals ({e_signal}).") # Separate statement
        # pass # Pass is implicit if nothing else is done
